    ...
    : StokesWaveGeneratorStrategy(object_name, input_db)
{
    ...
    if (!SAMRAI_MPI::getRank())
    {
        ...
        wave_stream.open("irregular_wave_generation_zone.txt", std::fstream::out);
...
double
IrregularWaveGenerator::getSurfaceElevation(double x, double time) const
{
    double eta = 0;
    for (int i = 0; i < d_num_waves; i++)
    {
...
double
IrregularWaveGenerator::getVelocity(double x, double z_plus_d, double time, int comp_idx) const
{
    if (comp_idx == 0)
    {
